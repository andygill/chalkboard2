...
animMain cb font sz (w,h) = do
        ...
        let background = pure $ boardOf $ withAlpha 0.2 yellow :: Active (Board (RGBA->RGBA))
            ...
            firstABC = mkActive tempAppear $ (move (-0.1,0) a) `over` (move (-0.03,-0.08) b) `over` (move (0.005,0.005) c)
            ...
            colorSquare = mkActive appear $ scale 0.095 $ choose (alpha yellow) transparent <$> square
            area = mkActive appear $ move (-0.4,0.35) $ makelbl aSP areaLabel
            secondABC = mkActive tempAppear $ move (0.15, 0.2) $ (move (0.06,0) a) `over` (move (0,0.06) b) `over` c `over` (move (0.05,-0.38) c)
            ...
            thirdABC = mkActive tempAppear $ (move (-0.15, -0.2) $ a `over` b) `over` (move (0.45,-0.36) a) `over` (move (0.4,0) b)
            ...
            finalABC = mkActive appear $ (move (-0.15, -0.2) a) `over` (move (0.03,-0.36) a) `over` (move (0.18,-0.2) b) `over` (move (0.4,0) b)
            formula = mkActive appear $ move (0.15,0.35) $ makelbl aSP formulaLabel
